const net = require("net"); // 网络模块，用于创建服务器和客户端
const http2 = require("http2"); // HTTP/2 模块，用于处理 HTTP/2 协议
const tls = require("tls"); // TLS/SSL 模块，用于加密通信
const cluster = require("cluster"); // 集群模块，用于创建多进程
const url = require("url"); // URL 模块，用于处理 URL
const crypto = require("crypto"); // 加密模块，用于生成加密数据
const fs = require("fs"); // 文件系统模块，用于读取和写入文件
const colors = require('colors'); // 颜色模块，用于在控制台输出带颜色的文本
const os = require("os"); // 操作系统模块，用于获取操作系统相关信息

// 错误处理函数
const errorHandler = error => {
    // console.log(error);
};
process.on("uncaughtException", errorHandler); // 处理未捕获的异常
process.on("unhandledRejection", errorHandler); // 处理未处理的 Promise 拒绝

process.setMaxListeners(0); // 设置最大监听器数量为无限制
require("events").EventEmitter.defaultMaxListeners = 0; // 设置事件监听器的默认最大数量

// 处理未捕获的异常
process.on('uncaughtException', function (exception) {});

if (process.argv.length < 7) {
    console.log(`方法：\nnode tls.js <目标> <时间> <速率> <线程> <代理文件>`); 
    process.exit();
}
...
var tipsz = tips1[Math.floor(Math.random() * tips1.length)];
